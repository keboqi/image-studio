...
def _normalize_ideogram4_lora_mode(mode: str | None) -> str:
    value = (mode or _runtime().IDEOGRAM4_LORA_OFF).strip()
    if value in _runtime().IDEOGRAM4_LORA_CHOICES:
        return value
    lower = value.lower()
    if lower in {"off", "none", "disabled", "disable"}:
        return _runtime().IDEOGRAM4_LORA_OFF
    if lower in {"runtime", "adapter", "runtime adapter"}:
        return _runtime().IDEOGRAM4_LORA_RUNTIME
    if lower in {"fused", "fuse", "fused in memory"}:
        return _runtime().IDEOGRAM4_LORA_FUSED
    return _runtime().IDEOGRAM4_LORA_OFF

def _normalize_ideogram4_lora_weight(weight_name: str | None) -> str:
    value = (weight_name or _runtime().IDEOGRAM4_REALISM_LORA_DEFAULT).strip()
    return value or _runtime().IDEOGRAM4_REALISM_LORA_DEFAULT

def _ideogram4_lora_short_name(weight_name: str) -> str:
    name = _runtime().os.path.basename(weight_name)
    return _runtime().os.path.splitext(name)[0].replace("Realism_Engine_Ideogram", "REI")
...
def _ideogram4_lora_signature(
    ...
    if mode == _runtime().IDEOGRAM4_LORA_FUSED:
        return (
            _runtime().IDEOGRAM4_REALISM_LORA_REPO,
            weight_name,
            round(float(conditional_strength), 6),
            round(float(unconditional_strength), 6),
        )
    return (_runtime().IDEOGRAM4_REALISM_LORA_REPO, weight_name)
...
def _ideogram4_unload_if_fused_lora_changed(
    ...
    model_key = _runtime().IDEOGRAM4_PIPELINE_MODEL_KEYS[_runtime()._normalize_ideogram4_pipeline(pipeline)]
    pipe = _runtime().model_mgr.get(model_key)
    if pipe is None:
        return
    state = getattr(pipe, "_image_studio_ideogram4_lora_state", None) or {}
    if state.get("mode") != _runtime().IDEOGRAM4_LORA_FUSED:
        return
    requested_sig = _ideogram4_lora_signature(
        mode, weight_name, conditional_strength, unconditional_strength
    )
    if mode != _runtime().IDEOGRAM4_LORA_FUSED or state.get("signature") != requested_sig:
        _runtime().log.info("Reloading Ideogram 4 pipeline to clear previous fused LoRA.")
...
def _ideogram4_load_lora_groups(weight_name: str) -> dict[str, Ideogram4LoRAGroup]:
    ...
    cache_key = (_runtime().IDEOGRAM4_REALISM_LORA_REPO, weight_name)
    with _ideogram4_lora_lock:
        ...
        try:
            path = hf_hub_download(
                repo_id=_runtime().IDEOGRAM4_REALISM_LORA_REPO,
            ...
            raise UserInputError(
                ...
                f"{_runtime().IDEOGRAM4_REALISM_LORA_REPO}: {exc}"
            ) from exc

        parts: dict[str, dict[str, torch.Tensor]] = {}
        alphas: dict[str, float] = {}
        weight_re = _runtime().re.compile(r"^(?P<module>.+)\.(?P<part>lora_[AB])\.weight$")
        alpha_re = _runtime().re.compile(r"^(?P<module>.+)\.(?:alpha|lora_alpha)$")
        ...
        for module_name, tensors in parts.items():
            ...
            if lora_a.ndim != 2 or lora_b.ndim != 2 or lora_a.shape[0] != lora_b.shape[1]:
                _runtime().log.warning("Skipping malformed LoRA tensors for %s", module_name)
                continue
        ...
        _runtime().log.info("Loaded %d Ideogram Realism Engine LoRA target(s).", len(groups))
        return groups
    ...
    with torch.no_grad():
        ...
        if class_name == "Fp8Linear":
            try:
                ...
                _runtime().log.warning("Could not fuse LoRA into FP8 layer %s: %s", group.module_name, exc)
                return False
...
def _ideogram4_apply_lora_to_transformer(
    ...
    for module_name, group in groups.items():
        ...
        if module is None or not _ideogram4_lora_group_matches(module, group):
            continue
        matched += 1
        if mode == _runtime().IDEOGRAM4_LORA_FUSED and _ideogram4_try_fuse_lora(module, group, strength):
            fused += 1
            continue
...
def _apply_ideogram4_lora(
    ...
    if mode == _runtime().IDEOGRAM4_LORA_OFF:
        if state.get("mode") != _runtime().IDEOGRAM4_LORA_FUSED:
        ...
        pipe._image_studio_ideogram4_lora_state = {"mode": _runtime().IDEOGRAM4_LORA_OFF}
        return ""
    ...
    if state.get("mode") == mode and state.get("signature") == signature:
        if mode == _runtime().IDEOGRAM4_LORA_RUNTIME:
    ...
    if mode == _runtime().IDEOGRAM4_LORA_FUSED and runtime_count:
        _runtime().log.warning(
    ...
    _runtime().log.info("Ideogram 4 LoRA ready: %s", status)
    return status
