...
def enable_ideogram4_cuda_fast_paths():
    ...
    if not _runtime().torch.cuda.is_available():
        return
    _runtime().torch.backends.cuda.matmul.allow_tf32 = True
    for backend_flag in (
        "enable_flash_sdp",
        ...
        flag_setter = getattr(_runtime().torch.backends.cuda, backend_flag, None)
        if flag_setter is None:
            continue
        try:
            flag_setter(True)
        except Exception as exc:
            _runtime().log.warning("Could not enable torch.backends.cuda.%s: %s", backend_flag, exc)
...
def make_ideogram4_sdpa_context():
    ...
    try:
        ...
        if not backends:
            return _runtime().contextlib.nullcontext()
        try:
...
def _import_ideogram4():
    ...
    pipeline_file = _runtime().os.path.join(_runtime().IDEOGRAM4_SRC_DIR, "ideogram4", "pipeline_ideogram4.py")
    if not _runtime().os.path.isfile(pipeline_file):
        raise UserInputError(
            "Ideogram 4 source is not installed. Run the quick start setup to clone "
            f"{_runtime().IDEOGRAM4_REPO} into {_runtime().IDEOGRAM4_DIR}."
        )
    if _runtime().IDEOGRAM4_SRC_DIR not in _runtime().sys.path:
        _runtime().sys.path.insert(0, _runtime().IDEOGRAM4_SRC_DIR)
...
def _normalize_ideogram4_pipeline(pipeline: str | None) -> str:
    ...
    if value in _runtime().IDEOGRAM4_PIPELINE_LABEL_TO_KEY:
        return _runtime().IDEOGRAM4_PIPELINE_LABEL_TO_KEY[value]
    if value in _runtime().IDEOGRAM4_PIPELINE_LABELS:
        return value
    lowered = value.lower()
    for label, key in _runtime().IDEOGRAM4_PIPELINE_LABEL_TO_KEY.items():
        if lowered == label.lower():
            return key
    return _runtime().IDEOGRAM4_DEFAULT_PIPELINE

def _ideogram4_pipeline_label(pipeline: str | None) -> str:
    return _runtime().IDEOGRAM4_PIPELINE_LABELS[_normalize_ideogram4_pipeline(pipeline)]
...
def _ideogram4_pipeline_config(pipeline: str | None = None):
    ...
    if pipeline == _runtime().IDEOGRAM4_PIPELINE_NVFP4:
        return config_cls(
            weights_repo=_runtime().IDEOGRAM4_NVFP4_WEIGHTS_REPO,
            conditional_index_filename=_runtime().IDEOGRAM4_NVFP4_CONDITIONAL_FILENAME,
            unconditional_index_filename=_runtime().IDEOGRAM4_NVFP4_UNCONDITIONAL_FILENAME,
            autoencoder_filename=_runtime().IDEOGRAM4_NVFP4_AUTOENCODER_FILENAME,
            tokenizer_repo=_runtime().IDEOGRAM4_NVFP4_CONFIG_REPO,
            text_encoder_config_repo=_runtime().IDEOGRAM4_NVFP4_CONFIG_REPO,
            tokenizer_subfolder=_ideogram4_config_subfolder("tokenizer"),
            text_encoder_subfolder=_ideogram4_config_subfolder("text_encoder"),
            text_encoder_weights_repo=_runtime().IDEOGRAM4_NVFP4_WEIGHTS_REPO,
            text_encoder_weights_filename=_runtime().IDEOGRAM4_NVFP4_TEXT_ENCODER_FILENAME,
        )
    if pipeline == _runtime().IDEOGRAM4_PIPELINE_FP8_NVFP4_UNCOND:
        return config_cls(
            weights_repo=_runtime().IDEOGRAM4_FP8_NVFP4_UNCOND_WEIGHTS_REPO,
            unconditional_weights_repo=_runtime().IDEOGRAM4_NVFP4_WEIGHTS_REPO,
            unconditional_index_filename=_runtime().IDEOGRAM4_NVFP4_UNCONDITIONAL_FILENAME,
        )
    return config_cls(weights_repo=_runtime().IDEOGRAM4_FP8_WEIGHTS_REPO)
...
def get_ideogram4_pipe(pipeline: str | None = None):
    pipeline = _normalize_ideogram4_pipeline(pipeline)
    model_key = _runtime().IDEOGRAM4_PIPELINE_MODEL_KEYS[pipeline]
    existing = _runtime().model_mgr.get(model_key)
    ...
    def factory():
        ...
        _runtime().log.info("Loading Ideogram 4 %s pipeline...", _ideogram4_pipeline_label(pipeline))
        pipe = mods["Ideogram4Pipeline"].from_pretrained(
            config=_ideogram4_pipeline_config(pipeline),
            device="cuda",
            dtype=_runtime().torch.bfloat16,
        )
        _runtime().log.info("Ideogram 4 %s pipeline ready.", _ideogram4_pipeline_label(pipeline))
        return pipe
