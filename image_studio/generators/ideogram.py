...
def run_ideogram4_generate(
    ...
    width, height = _runtime().validate_ideogram4_dims(width, height)
    pipeline = _runtime()._normalize_ideogram4_pipeline(pipeline)
    lora_mode = _runtime()._normalize_ideogram4_lora_mode(lora_mode)
    lora_weight = _runtime()._normalize_ideogram4_lora_weight(lora_weight)
    lora_cond_strength = float(lora_cond_strength or 0.0)
    lora_uncond_strength = float(lora_uncond_strength or 0.0)
    _runtime()._ideogram4_unload_if_fused_lora_changed(
        pipeline,
        lora_mode,
        lora_weight,
        lora_cond_strength,
    ...
    if upsampler != _runtime().IDEOGRAM4_UPSAMPLE_NONE:
        ...
        upsampled_prompt = _runtime()._ideogram4_upsample_prompt(
    ...
    final_prompt = _runtime()._ideogram4_normalize_caption_for_model(
        upsampled_prompt,
        width,
        height,
    ...
    progress(0.2, desc=f"Loading Ideogram 4 {_runtime()._ideogram4_pipeline_label(pipeline)}...")
    pipe = _runtime().get_ideogram4_pipe(pipeline)
    lora_status = _runtime()._apply_ideogram4_lora(
        pipe,
        lora_mode,
        lora_weight,
        lora_cond_strength,
    ...
    preset_key = _runtime().IDEOGRAM4_SAMPLER_PRESETS.get(
        sampler_preset or "",
        _runtime().IDEOGRAM4_SAMPLER_PRESETS["Turbo - 12 steps"],
    )
    ...
    pipe_kwargs = dict(
        ...
        std=preset.std,
        seed=seed,
        raise_on_caption_issues=upsampler != _runtime().IDEOGRAM4_UPSAMPLE_NONE,
        callback_on_step_end=step_callback,
    )
    if pid_enabled:
        pipe_kwargs["output_type"] = "latent"

    with _runtime().make_ideogram4_sdpa_context():
        try:
    ...
    diffusion_elapsed = _runtime().time.perf_counter() - t_diff
    _runtime().log.info("Ideogram diffusion (%s) complete in %.2fs", sampler_preset, diffusion_elapsed)

    pid_parts: list[str] = []
    if pid_enabled:
        progress(0.82, desc="Preparing Ideogram PiD 4x decode...")
        result, pid_elapsed, pid_ckpt_type, pid_out_w, pid_out_h = _runtime()._decode_ideogram4_with_pid(
            image_or_latents,
            final_prompt,
            source_prompt,
            width,
        ...
        pid_parts = [
            ...
            f"{_runtime()._pid_checkpoint_label(_runtime().PID_BACKBONE_IDEOGRAM4, pid_ckpt_type)} 4x",
    ...
    status = _runtime().ok_status(
        ...
        *(pid_parts or [f"{width}x{height}"]),
        f"Ideogram 4 {_runtime()._ideogram4_pipeline_label(pipeline)}",
        sampler_preset,
        *cfg_parts,
        f"upsampler {upsampler}",
    )
    preview_path, status, raw_path = _runtime().finalize_image_result("ideogram4", result, status, seed, always_seed=True)
    _runtime()._write_ideogram4_prompt_metadata(
        raw_path,
        {
            "source_prompt": source_prompt,
            "upsampled_prompt": upsampled_prompt,
            "model_prompt": final_prompt,
            "editor_prompt": _runtime()._ideogram4_editor_prompt_from_candidates(
                upsampled_prompt,
                final_prompt,
                source_prompt,
            ),
