...
def run_generate(
    ...
    pipe = _runtime().get_gen_pipe()
    gen = _runtime().make_cuda_generator(seed)
    kwargs = dict(prompt=prompt, width=width, height=height,
                  num_inference_steps=_runtime().LIGHTNING_STEPS, true_cfg_scale=cfg)
    ...
    if pid_enabled:
        result, elapsed, pid_ckpt_type, pid_out_w, pid_out_h = _runtime()._decode_qwen_with_pid(
        ...
        return _runtime().finalize_image_result("gen_pid", result, status, seed)

    progress(0.3, desc="Generating...")
    result, elapsed = _runtime().timed_result(lambda: pipe(**kwargs).images[0])
    status = _runtime().ok_status(elapsed, f"{width}x{height}", f"CFG {cfg}")
    return _runtime().finalize_image_result("gen", result, status, seed)
...
def run_edit(img1: Any, img2: Any, img3: Any, prompt: str, neg_prompt: str, cfg: float, seed: int, progress=NO_PROGRESS) -> tuple[Image.Image, str]:
    ...
    progress(0.3, desc="Generating...")
    result, elapsed = _runtime().timed_result(lambda: pipe(**kwargs).images[0])
    status = _runtime().ok_status(elapsed, f"{len(images)} image(s)", f"CFG {cfg}")
    return _runtime().finalize_image_result("edit", result, status, seed)
